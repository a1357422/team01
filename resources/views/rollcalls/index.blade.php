...
<div class="function">
    ...
    <div>
        <form action="{{ url('rollcalls/dormitory') }}" method='POST'>
            ...
            @endif
            <input type="submit" value="查詢" class="btn btn-primary" style="margin-bottom: 1em;" />
            @csrf
        </form>
        @if($display == 1 || $display == 2)
        ...
        <a href="{{ route('rollcalls.presence') }}"class="btn btn-warning" style="margin-top: 1em; margin-bottom: 1em;">未到人員</a>
            ...
            <table class="table">
                ...
                <tr class='column_center'>
                    ...
                    <td align="center" valign="center">
                        <font color=green>{{ $rollcall->presence = "V" }}</font>
                    </td>
                    @else
                    <td align="center" valign="center">
                        <font color=red>{{ $rollcall->presence = "X" }} </font>
                    </td>
                    @endif
                    @if ($rollcall->leave === 1)
                    <td align="center" valign="center">
                        <font color=green>{{ $rollcall->leave = "V" }}</font>
                    </td>
                    @else
                    <td align="center" valign="center">
                        <font color=red>{{ $rollcall->leave = "X" }} </font>
                    </td>
                    @endif
                    @if ($rollcall->late === 1)
                    <td align="center" valign="center">
                        <font color=green>{{ $rollcall->late = "V" }}</font>
                    </td>
                    @else
                    <td align="center" valign="center">
                        <font color=red>{{ $rollcall->late = "X" }} </font>
                    </td>
                    @endif
                    @if ($rollcall->identify === 1)
                    <td align="center" valign="center">
                        <font color=green>{{ $rollcall->identify = "V" }}</font>
                    </td>
                    @else
                    <td align="center" valign="center">
                        <font color=red>{{ $rollcall->identify = "X" }} </font>
                    </td>
                    @endif
                    <td align="center" valign="center">
                        <font color=blue><a href="{{ route('rollcalls.show',[ 'id'=>$rollcall->id ]) }}" class="btn btn-primary">詳細資料</a></font>
                    </td>
                    @if ($rollcall->presence == "V")
                    <td>
                        <font color=blue><a href="{{ route('rollcalls.edit',['id'=>$rollcall->id,'presence'=>0]) }}" class="btn btn-secondary">未到</a></font>
                    </td>
                    @else
                    <td>
                        <font color=blue><a href="{{ route('rollcalls.edit',['id'=>$rollcall->id,'presence'=>1]) }}" class="btn btn-secondary">補點</a></font>
                    </td>
                    @endif
                    <td>
